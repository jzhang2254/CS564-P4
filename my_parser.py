...
def parseJson(json_file):
    ...
    Auction = open("Auction.dat", 'a+')
    Category = open("Category.dat", 'a+')
    Bid = open("Bid.dat", 'a+')
    Seller = open("Seller.dat", 'a+')
    Bidder = open("Bidder.dat", 'a+')
    
    with open(json_file, 'r') as f:
        items = loads(f.read())['Items'] # creates a Python dictionary of Items for the supplied json file
        for item in items:
            Item.write(item["ItemID"]+'|'+'\"'+item["Name"].replace("\"", "\"\"")+'\"'+'|'+'\"'+item["Seller"]["UserID"]+'\"'+'|'+'\"'+str(item["Description"]).replace("\"", "\"\"")+'\"'+'\n')
            ...
            if(item.get("Bids")!= None):
                for bid in item["Bids"]:
                    ...
                    Bidder.write(bid["Bid"]["Bidder"]["UserID"]+'|'+bid["Bid"]["Bidder"]["Rating"]+'|'+strquote(bid["Bid"]["Bidder"].get("Location","None"))+'|'+strquote(bid["Bid"]["Bidder"].get("Country", "None",))+'\n')
                    #Bidder.write(bid["Bid"]["Bidder"]["UserID"]+'|'+bid["Bid"]["Bidder"]["Rating"]+'|'+"lmao"+'|'+"lmao"+'\n')
            
            Seller.write(item["Seller"]["UserID"]+'|'+item["Seller"]["Rating"]+'|'+strquote(item["Location"])+'|'+strquote(item["Country"])+'\n')
            #Seller.write(item["Seller"]["UserID"] + '|' +item["Seller"]["Rating"] +'|'+"lmao"+'|'+"lmao"+'\n')
